...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ### YOUR CODE HERE
	    #embedding = tf.get_variable("embedding", [self.vocab.total_words, self.config.embed_size], initializer = tf.random_uniform_initializer(-1.0,1.0)) 
            #W1 = tf.get_variable("W1",[2 * self.config.embed_size, self.config.embed_size], initializer=tf.random_uniform_initializer(-0.05, 0.05)) 
            #b1 = tf.get_variable("b1", [1, self.config.embed_size], initializer = tf.constant_initializer(0))
        ...
        with tf.variable_scope('Projection'):
            ### YOUR CODE HERE
            #U = tf.get_variable("U",[self.config.embed_size, self.config.label_size], initializer=tf.random_uniform_initializer(-0.05, 0.05))
            #bs = tf.get_variable("bs", [1, self.config.label_size], initializer = tf.constant_initializer(0))
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            tmp = self.vocab.encode(node.word)
            curr_node_tensor = tf.expand_dims(tf.gather(embedding, tmp),0) 
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            lf_tensor = tf.concat(1,[node_tensors[node.left], node_tensors[node.right]])
            curr_node_tensor = tf.matmul(lf_tensor, W1) + b1
