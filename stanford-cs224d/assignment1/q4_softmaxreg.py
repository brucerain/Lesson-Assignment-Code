...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sent = [wordVectors[tokens[i]] for i in sentence]
    sentVector = np.sum(sent,axis=0)/(len(sentence)) 
    ### END YOUR CODE
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    dout = prob.copy()
    dout[range(N), labels]-=1
    dout/=N
    grad = features.T.dot(dout) + regularization * weights
