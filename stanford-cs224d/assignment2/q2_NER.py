...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      w2v = tf.convert_to_tensor(self.wv, dtype=tf.float32) # shape: [len(wv, embed_size)]
      train_input = tf.reshape(self.input_placeholder,[-1])  # flatten input_batch  
      embed = tf.nn.embedding_lookup(w2v, train_input)
      window = tf.reshape(embed,[-1, self.config.window_size * self.config.embed_size]) # [batch_size, window_size * embed_size]
      ### END YOUR CODE
      return window
