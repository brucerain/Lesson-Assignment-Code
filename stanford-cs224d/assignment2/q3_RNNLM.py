...
class RNNLM_Model(LanguageModel):
  ...
  def load_data(self, debug=True):
    ...
    # encoded_train/valid/test: an array of wordid (a,b,c <eos>, d, e,f) 
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      embedding = tf.Variable(tf.random_uniform([len(self.vocab),self.config.embed_size], -1.0, 1.0))
      #train_input = tf.reshape(tf.transpose(self.input_placeholder),[-1])  #[num_step,batch_size] 
      embed = tf.nn.embedding_lookup(embedding, self.input_placeholder)
      inputs = tf.split(1 , self.config.num_steps, embed)
  ...
  def add_projection(self, rnn_outputs):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope("projection",initializer=tf.random_normal_initializer() ) as scope:
      U = tf.get_variable("U",(self.config.hidden_size,len(self.vocab))) 
      b_2 = tf.Variable(tf.zeros((len(self.vocab),)), "b_2")
         
      outputs = [] 
      for i in xrange(len(rnn_outputs)):
        tmp = tf.matmul(rnn_outputs[i],U) + b_2
        outputs.append(tmp)
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    loss = sequence_loss([output],[target],[weights])
    ### END YOUR CODE
    return loss
  ...
  def run_epoch(self, session, data, train_op=None, verbose=10):
    ...
    for step, (x, y) in enumerate(
      # "data" the same as self.encode_train : a sequence of word id. 
